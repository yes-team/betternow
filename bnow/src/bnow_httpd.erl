...
-export([start_link/0]).
-export([init/3, handle/2, terminate/2, gc/0]).
-include_lib("kernel/include/file.hrl").
-include_lib("cowboy/include/http.hrl").
-include("defined.hrl").
-compile(export_all).
-record(state, {appdir, basedir, gateport}).

init({_Any, http}, Req, []) ->
    {ok, Port} = application:get_env(bnow, gate_port),
    timer:apply_interval(timer:hours(1),  bnow_httpd, gc, []),
	{ok, Req, #state{appdir=root_path(filename:absname(bnow:app_dir())), 
	    gateport = integer_to_list(Port),
	    basedir = root_path(filename:absname(filename:join(priv_dir(bnow), "web")))}}.

handle(#http_req{path= [<<"plugin">> | [H|_T] ] } = R, S) ->
    apply( list_to_atom( binary_to_list( H ) ) , handle , [R,S] );

handle(#http_req{path= [<<"app">> | [H|T] ] } = R, S) ->
    run(S#state.appdir, [H | [<<"web">>|T]], R, S, H);
    
handle(#http_req{path= T } = R, S) ->
    run(S#state.basedir, T, R, S, <<>>);
...
start_link()->
    {ok, {tcp, Fcgi_ip, Fcgi_port}} = application:get_env(bnow,php_fcgi),
    ex_fcgi:start(bnow_fcgi, Fcgi_ip, Fcgi_port),
    
    Dispatch = [ {'_', [ 
            { [<<"login">> , '...'] , bnow_oauthc , []  },
            { [<<"api">>, <<"live">>] , bnow_webflow, []},
            { [<<"api">>, '...'] , bnow_webapi, []},
            { '_', ?MODULE, []}
        ]} ],

    {ok, {Ip, Port}} = application:get_env(bnow,web_listen),
...
read_file_info(Filepath)->
    case ets:lookup(bnow_cache, {file_info, Filepath}) of
        [] ->
            Ret = file:read_file_info(Filepath),
            ets:insert(bnow_cache, {{file_info, Filepath}, Ret}),
            Ret;
        [{_, Ret} | _] -> Ret
    end.

read_file(Filepath)->
    case ets:lookup(bnow_cache, {file, Filepath}) of
        [] ->
            Ret = file:read_file(Filepath),
            ets:insert(bnow_cache, {{file, Filepath}, Ret}),
            Ret;
        [{_, Ret} | _] -> Ret
    end.

run(Base_dir, Path, R, S, APP)->
    case  bnow_global:fetch(running) of
        true ->
    ...
    case read_file_info(Filepath) of
        {ok, #file_info{type=regular, access=Access}=F} when ( Access =:= read ) or ( Access =:= read_write ) ->
            case filename:extension(Filepath) of
                <<".php">> ->
                    run_php(R, [
                    {<<"BNOW_APIGATE">>, S#state.gateport},
                    {<<"BNOW_APP">>, APP},
                    {<<"BNOW_APP_NAME">>, bnow_var:get_app_env(bnow, application)},
                    {<<"BNOW_API_HOST">>, bnow_var:get_app_env(bnow, api_host)},
                    {<<"BNOW_APPDIR">>, S#state.appdir},
                    {<<"BNOW_BASEDIR">>, S#state.basedir},
...
gc()->
    gc_session(mnesia:dirty_first(bnow_session), bnow_timer:now()).

gc_session('$end_of_table', _)->ok;
gc_session(K, Now)->
    case mnesia:dirty_read(bnow_session, K) of
        [S|_] ->
            Next = mnesia:dirty_next(bnow_session, K),
            if 
                Now - S#bnow_session.update_time > S#bnow_session.ttl ->
                    mnesia:dirty_delete(bnow_session, K);
                true -> ok
            end,
            gc_session(Next, Now);
        _ -> ok
